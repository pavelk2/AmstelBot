...
module.exports = {
    getPlaces: function(location, venue_query, callback) {
        var api = this;
        var extension = "nearbysearch/json?location=" + location.lat + "," + location.long + "&rankby=distance&type=restaurant&opennow=true&query=" + encodeURI(venue_query) + "&key=" + API_KEY;
    ...
    getPhoto: function(photoreference) {
        return 'https://maps.googleapis.com/maps/api/place/' + "photo" + "?maxwidth=500&maxheight=500&photo_reference=" + photoreference + "&key=" + API_KEY
    },
    getDetails: function(place_id, callback) {
        var api = this;
        var extension = "details/json?placeid=" + place_id + "&key=" + API_KEY;
